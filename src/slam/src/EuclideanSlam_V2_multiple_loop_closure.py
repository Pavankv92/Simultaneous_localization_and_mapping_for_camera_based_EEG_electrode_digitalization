...
icp_fitness_threshold = 0.90 # 0.80
vSet = pcViewSet()
    ...
    for n in range(0, (num_frames), step_to_skip):
        ...
        if loop_closed == True:
            if loop_view_id in candidates:
                print("loop view id : {} is already used".format(loop_view_id))
...
if __name__ == "__main__":
    ...
    #cluster_centers = vSet.clusterTheElectrodes(ground_truth_electrodes_in_first_camera_frame)
    vSet.getClusterMetrics_kMeans(ground_truth_electrodes_in_first_camera_frame, estimated_electrodes_in_first_camera_frame,21,show=True)
