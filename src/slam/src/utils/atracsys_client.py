...
class attracsysClient(object):
    ...
    def __init__(self, address, port, locatorname, format, timeout=25):
        ...
        print("Attracsys client initialized")
    ...
    def record_marker_ignore_visibility(self):
        matrix = self.get_transformMatrix()
        if np.array_equal(np.eye(4), matrix):
            print('Locator ' + self.locatorname + " is not visible.")
        else:
            return matrix
        return np.eye(4)
    ...
    def record_marker(self):
        ...
        response = ''
        while not transformation and response != 'q':
            matrix = self.get_transformMatrix()
            if np.array_equal(np.eye(4), matrix):
                response = raw_input('Locator ' + self.locatorname + " is not visible. Replace it and hit enter.")
            else:
                transformation = True
                return matrix
    ...
    def record_marker_head_band(self):
        matrix = self.get_transformMatrix()
        if np.array_equal(np.eye(4), matrix):
            print('Locator ' + self.locatorname + " is not visible.")
        else:
...
if __name__ == "__main__":
    s = attracsysClient('134.28.45.17',5000,'bluestylustip','FORMAT_MATRIXROWWISE')
