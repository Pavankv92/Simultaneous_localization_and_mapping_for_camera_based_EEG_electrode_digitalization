...
def synchronize_trajectory(ground_truth_camera_pose_dict, yolo_electrodes_list, path_to_save_ground_truth_camera_poses):
    ...
    np.save(str(path_to_save_ground_truth_camera_poses), ground_truth_camera_bad_poses_removed_list)
...
def plot(inertia, x_label, y_label, title):
    ...
    plt.title(str(title))
    fig.savefig(base_dir + "final_images/{}_cluster_comparison".format(str(title)))
    plt.show()
...
if __name__ == "__main__":
    ...
    base_dir_cloud = '/media/pallando/share/students/Vishwanath/master_arbeit_data/hand_trajectory/static_phantom_head/CA_124/7/'
    base_dir = "/home/Vishwanath/catkin_ws/src/master_arbeit/src/slam/src/data/hand_trajectory/static_phantom_head/beta_10/CA_124_7/"
    ...
    # just for saving after figuring out skip
    #"""
    electrode_positions_yolo = base_dir_cloud + "depth_variance/10/electrode_position_from_text_file.npy"
    electrodes_dict = getElectrodes_from_array(electrode_positions_yolo, skip=1)
    ground_truth_camera = prepare_ground_truth_poses(ground_truth_camera_pose_path,11)
    ...
    # cluster center comparison / inertia is done is V2.
    """
    ...
    for i in range(len(depth_offset)):
        ...
        electrodes_dict = getElectrodes_from_array(electrode_positions_yolo, skip=1)
        ground_truth_camera = prepare_ground_truth_poses(ground_truth_camera_pose_path,12)
        ground_truth_camera_bad_poses_removed_list, electrodes_list = synchronize_trajectory(ground_truth_camera, electrodes_dict, ground_truth_camera_bad_poses_removed)
        estimated_electrodes_in_first_camera_frame = map_electrodes(ground_truth_camera_bad_poses_removed_list, electrodes_list)

        cluster_centers, inertia = vSet.clusterTheElectrodes_KMeans_depth(estimated_electrodes_in_first_camera_frame,50,path_to_save,show=False)
    ...
    plot(inlier_rmse_list, "depth(mm)", "RMSE", "CS_301_RED_14")
