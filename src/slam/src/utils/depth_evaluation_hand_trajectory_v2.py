...
def plot(inertia, x_label, y_label, title):
    ...
    plt.title(str(title))
    plt.show()
    fig.savefig(base_dir + "final_images/{}_depth".format(str(title)))
...
if __name__ == "__main__":
    ...
    base_dir_cloud = '/media/pallando/share/students/Vishwanath/master_arbeit_data/hand_trajectory/static_phantom_head/CA_124/6/'
    base_dir = "/home/Vishwanath/catkin_ws/src/master_arbeit/src/slam/src/data/hand_trajectory/static_phantom_head/CA_124_6/"
    ...
    for i in range(len(depth_offset)):
        ...
        for i in range(len(sync_skip)):
            ...
            inertia_list_per_depth.append(inertia)
            #print("inertia_per_depth", inertia)
            #pc_yolo_base_pub.publish(convertCloudFromOpen3dToRos(estimated_electrodes_in_first_camera_frame))
        #inertia_min = np.amin(inertia_list_per_depth)
        inertia_min = min(inertia_list_per_depth)
        print("inertia_min", inertia_min)
        idx_min = inertia_list_per_depth.index(inertia_min)
        #print("idx_min", idx_min)
        skip_min = sync_skip.index(idx_min)
        print("skip_min", skip_min)
        inertia_list.append(inertia_min)
        skip_list.append(skip_min)
        #print(idx_list)
    ...
    plot(inertia_list, "depth(mm)", "inertia(m.sq)", "CS_301_RED_14")
