...
class PCLTransformer:
    ...
    def __init__(self, markerFilename,
                  src_img_size = None, dest_img_size = None,
    ...
    def transform_tf_head_to_kin(self, tf, robPose) :
    ...
    def transform_point_head_to_kin(self, point, robPose):
        # in:point in head coordinate frame
        # robPose = base_T_kin , hand eye is included
        kin_P_point = np.dot(np.linalg.inv(robPose), point)
        return kin_P_point

    #transfer all gt electrodes (4x1) to single robot pose hence single kinect pose kin_T_allelectrodes
    def tranform_points_head_to_kin(self, robPose, points= None):
        if points is None:
            points = self.cap_points
        return [self.transform_point_head_to_kin(point, robPose) for point in points]
    #everything untill here is done in robot base frame
    ...
    def build_model_from_points(self, points = None, robotPose = None) :
        if points is None:
            points = self.cap_points
    ...
    def bloat_model(self, model, distance, center):
    ...
    def remove_invisible_points(self, model, points, max_angle, origin = np.asarray([0, 0, 0, 1])):
        res = []
        for point in points:
    ...
    @staticmethod
    def point_in_vertex(point, vertex):
        for v in vertex:
            same = True
    ...
    def electrode_sharp_angled(self, origin, point, points, max_angle):
    ...
    def too_far_away(self, origin, point, points, mean_fraction =0.95):
        mean = 0
        max_p = -float('Inf')
        min_p = float('Inf')
        for p in points:
    ...
    def intersect(self, origin, point, model):
        for vertex in model:
            epsilon = 0.00001
    ...
    def read_marker_in_head_from_file(self, filename):
        self.cap_points = []
        self.cap_names = []
    ...
    def remove_outside_boxes(self, boxes):
        result = []
        for box in boxes:
    ...
    def process_poses(self, filename, electrode_pos_fn, poses, idx, img_fns = None, model_bloat = 0.0, max_angle=np.pi, head_tfs_fn = None):
    ...
    def save_points_to_file(self, filename, point_list, img_filename, img_index):
    ...
    def save_head_tf(self, filename, tf, img_filename, img_index):
        with open(filename, 'a') as file:
            strings = [str(img_index), img_filename]
    ...
    def save_boxes_to_file(self, filename, box_list, img_filename, img_index):
        with open(filename, 'a') as file:
    ...
    def read_robot_poses(self, pose_list):
        poses = np.load(str(pose_list), allow_pickle=True)
        idx_list = [x for x in np.arange(0, len(poses), 1)]
        return poses, idx_list 